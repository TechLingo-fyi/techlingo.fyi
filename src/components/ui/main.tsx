...
<section className="container mx-auto py-8 px-4 md:px-6">
      ...
      <div className="flex mt-12">
        ...
        <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 flex-grow overflow-auto">
            {
              Array.from(sortedGroupedLingos.entries()).map(([letter, lingos]) => (
                  <>
                  {
                    lingos.map((lingo,idx) => (
                      <a href={lingo.term}>
                        <Card className="cursor-pointer hover:bg-gray-50 hover:shadow-lg transition-all duration-200" id={lingo.term}>
                          <CardHeader>
                            {idx === 0 && <span id={letter}></span>}
                            <h2 className="text-lg font-semibold">{lingo.term} <small className="text-sm text-gray-400 font-normal">{lingo.definitions[0].expanded}</small></h2>
                          </CardHeader>
                          <CardContent>
                            <p className="text-base text-gray-700">{lingo.definitions[0].definition}</p>
                          </CardContent>
                        </Card>
                        </a>
                    ))
